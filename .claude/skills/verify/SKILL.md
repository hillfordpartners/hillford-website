...
- **`window.scrollTo()`/`scrollBy()` via `javascript_tool` don't move the page in the claude-in-chrome MCP tab, and `IntersectionObserver` doesn't reliably re-fire on subsequent crossings there either** (confirmed: a fresh ad-hoc observer never fired even once across several real position changes). If a feature depends on scroll position (sticky crossfade, scroll-triggered reveals), drive scrolling with the `computer` tool's `scroll` action (real wheel-event simulation — this one works and is what the reveal-up system already relies on), not JS `scrollTo`. And prefer a `scroll` event + `requestAnimationFrame` listener over `IntersectionObserver` for anything that needs continuous position tracking (multiple crossings, both directions) rather than a one-shot visibility check — it's more standard for that use case anyway, and doesn't depend on IO behavior that's flaky in this harness.
...
- **`astro build` itself also spawns a `workerd.exe`** now that the Cloudflare adapter is installed (it prerenders through a local Workers runtime for `dist/server/.prerender/.dev.vars` secrets binding) — not just `wrangler dev`. So run `taskkill //F //IM workerd.exe` after every `astro build` too, not only after `wrangler dev`, or the next build/screenshot pass can hit the same directory-lock issue.
